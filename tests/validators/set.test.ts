...
describe('SetValidator', () => {
	...
	test.each([123, 'foo', [], {}, new Map()])("GIVEN a value which isn't a set (%p) THEN throws ValidationError", (input) => {
		expectError(() => predicate.parse(input), new ValidationError('s.set(T)', 'Expected a set', input));
	});

	test.each(['1', 'a', 'foo'])('GIVEN a set with string value (%p) THEN returns the given set', (input) => {
		const set = new Set([input]);

		expect(predicate.parse(set)).toStrictEqual(set);
	});

	test.each([123, [], {}])('GIVEN a set with non-string value (%p) THEN throw CombinedError', (input) => {
		const set = new Set([input]);
